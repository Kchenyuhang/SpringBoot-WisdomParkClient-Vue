...
export default {
  ...
  created() {
    this.selectunderwary();
  },
  components: { Dialog },
  methods: {
    async selectunderwary() {
      this.data = {
    ...
    async filish(index) {
      ...
      this.url = this.GLOBAL1.baseUrl + "/transaction/finshOrder";
      this.filishs = await API.init(this.url, this.data, "post");
      console.log(this.filishs);
...
@import "../../../assets/scss/errands/orders/Runall.scss";
</style>
